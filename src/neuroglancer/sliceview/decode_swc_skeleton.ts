 ...
 * modified from https://github.com/JaneliaSciComp/SharkViewer/blob/d9969a7c513beee32ff9650b00bf79cda8f3c76a/html/js/sharkviewer_loader.js 
 */
function parseSwc(swc_str: string) {
  ...
  each(swc_input_ar, function (e) {
    ...
    if (match) {
      let point = swc_objects_ar[parseInt(match[1]) - 1] = new PointObj();
      point.type     = parseInt  (match[2]);
      point.x        = parseFloat(match[3]);
      point.y        = parseFloat(match[4]);
      point.z        = parseFloat(match[5]);
      point.radius   = parseFloat(match[6]);
      point.parent   = parseInt  (match[7]) - 1;
    }
  });
