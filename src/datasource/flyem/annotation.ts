...
/**
 * Fixes an annotation after structured cloning.
 * Structured cloning (e.g., via postMessage) can convert typed arrays to regular arrays.
 * This function restores them to the proper typed array format.
 */
function fixAnnotationAfterStructuredCloning(
  annotation: any,
): Annotation | null {
  if (annotation == null) return null;

  // Fix relatedSegments (BigUint64Array[])
  if (annotation.relatedSegments) {
    annotation.relatedSegments = annotation.relatedSegments.map(
      (segments: any) =>
        segments instanceof BigUint64Array
          ? segments
          : new BigUint64Array(
  ...
  // Fix geometry based on annotation type
  switch (annotation.type) {
    case AnnotationType.POINT:
      if (!(annotation.point instanceof Float32Array)) {
        annotation.point = new Float32Array(annotation.point);
      }
      break;
    case AnnotationType.LINE:
    case AnnotationType.AXIS_ALIGNED_BOUNDING_BOX:
      if (!(annotation.pointA instanceof Float32Array)) {
        annotation.pointA = new Float32Array(annotation.pointA);
      }
      if (!(annotation.pointB instanceof Float32Array)) {
        annotation.pointB = new Float32Array(annotation.pointB);
      }
      break;
    case AnnotationType.ELLIPSOID:
      if (!(annotation.center instanceof Float32Array)) {
        annotation.center = new Float32Array(annotation.center);
      }
      if (!(annotation.radii instanceof Float32Array)) {
        annotation.radii = new Float32Array(annotation.radii);
      }
      break;
    case AnnotationType.POLYLINE:
      if (annotation.points) {
        annotation.points = annotation.points.map((p: any) =>
...
class TEllipsoid extends TAnnotationBase implements Ellipsoid {
  center: Float32Array = new Float32Array(3);
  radii: Float32Array = new Float32Array(3);
  type: AnnotationType.ELLIPSOID = AnnotationType.ELLIPSOID;
}
...
export class EllipsoidAnnotation extends WithFlyEMProp(TEllipsoid) {}
...
export class AnnotationFacade {
  ...
  roundPos() {
    if (this.annotation.type === AnnotationType.POINT) {
      this.annotation.point = Float32Array.from(
        this.annotation.point,
        (x) => Math.round(x),
      );
    } else if (this.annotation.type === AnnotationType.LINE) {
      this.annotation.pointA = Float32Array.from(
        this.annotation.pointA,
        (x) => Math.round(x),
      );
      this.annotation.pointB = Float32Array.from(
        this.annotation.pointB,
        (x) => Math.round(x),
      );
    } else if (this.annotation.type === AnnotationType.ELLIPSOID) {
      this.annotation.center = Float32Array.from(
        this.annotation.center,
        (x) => Math.round(x),
      );
    }
  }
  ...
  get checked() {
    return (
      (this.ext && this.ext.verified) || (this.prop && this.prop.checked) || false
    );
  }
...
export function typeOfAnnotationId(id: AnnotationId) {
  if (
    ...
    id.match(/^-?\d+_-?\d+_-?\d+--?\d+_-?\d+_-?\d+-Ellipsoid$/) ||
    id.match(/^Sp-?\d+_-?\d+_-?\d+_?\d+_-?\d+_-?\d+/)
  ) {
    return AnnotationType.ELLIPSOID;
  } else {
  ...
  if (!key) {
    switch (annotation.type) {
      ...
      case AnnotationType.LINE:
        ...
        break;
      case AnnotationType.ELLIPSOID:
        key = `Sp${annotation.center[0]}_${annotation.center[1]}_${annotation.center[2]}_${annotation.radii[0]}_${annotation.radii[1]}_${annotation.radii[2]}`;
        break;
    }
  }
